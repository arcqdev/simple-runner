...
| `kodo logs` | `kodo/cli/_subcommands.py`, `kodo/viewer.py` | `partial` | TS opens a viewer and serves logs, but the viewer UX/event rendering is still much simpler than Python |
| `kodo issue` | `kodo/cli/_subcommands.py` | `partial` | TS builds archive and issue URL, but interactive copy and browser-opening behavior differ |
...
| `kodo update` | `kodo/cli/_subcommands.py` | `partial` | TS shells out to `uv tool upgrade ...`; final parity around messaging and failure handling remains pending |
...
| `kodo logs [logfile] [--port PORT]` | Default port `8080`; missing file is fatal | `partial` | TS accepts same arguments, but viewer output differs |
| `kodo issue [run_id] [--project PROJECT] [--no-open]` | Default project `.`; if `run_id` omitted, prompt from runs | `partial` | TS supports current syntax; UX copy differs and browser-open flow is not a byte-for-byte match |
...
| `kodo update` | No flags; requires `uv` | `partial` | |
...
|---|---|---|---|
| Banner before interactive/non-JSON runs | `kodo/cli/_ui.py`, `_main.py` | `pending` | TS does not yet mirror the Python banner/copy |
...
| `issue` description prompt | `kodo/cli/_subcommands.py` | `partial` | TS supports issue creation, but prompt copy and browser/open-folder flow differ |
...
| `kodo logs` port collision hint | `kodo/viewer.py` | `partial` | Python prints `Hint: try a different port with --port <n+1>`; TS needs to preserve this exact diagnostic family |
| Next-step tips after run/resume | `kodo.tips` hooks from `_main.py` | `pending` | Not implemented in TS |
...
| Trace upload affordances | `viewer.html`, `trace_upload.py` | `pending` | |
...
| Current TS prompt copy | `intentional-deviation` for now | The shell flow is functional, but prompt/banner wording is still scheduled work rather than a hidden omission |
