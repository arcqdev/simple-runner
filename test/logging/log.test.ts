  ...
  it("tracks stage completion and agent session ids", () => {
    ...
    writeEvents(logFile, [
      ...
      { event: "stage_start", stage_index: 2 },
      {
        ...
        provider_thread_id: "provider-smart",
        server_session_id: "server-smart",
        usage_raw: { costUsd: 0.02, inputTokens: 21, outputTokens: 13 },
      },
      {
        event: "agent_run_end",
        agent: "worker_smart",
      ...
      },
      {
        ...
        provider: "gemini",
        provider_env_vars: ["GEMINI_API_KEY", "GOOGLE_API_KEY"],
        provider_thread_id: "provider-fast",
        server_session_id: "server-fast",
        usage_raw: { costUsd: 0.01, inputTokens: 8, outputTokens: 5 },
      },
      {
        event: "agent_run_end",
        agent: "worker_fast",
    ...
    expect(state?.agentSessionDetails).toEqual({
      worker_fast: {
        acpBackend: "opencode",
        provider: "gemini",
        providerEnvVars: ["GEMINI_API_KEY", "GOOGLE_API_KEY"],
        providerThreadId: "provider-fast",
        serverSessionId: "server-fast",
        sessionId: "chat-xyz",
      },
      worker_smart: {
        acpBackend: "gemini",
        provider: "gemini",
        providerEnvVars: ["GEMINI_API_KEY", "GOOGLE_API_KEY"],
        providerThreadId: "provider-smart",
        serverSessionId: "server-smart",
        sessionId: "ses-abc",
      },
    });
