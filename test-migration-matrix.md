...
The following test areas remain explicitly deferred after cutover and are not hidden gaps:

- `tests/knowledge/*`:
  deferred until the TS knowledge subsystem exists; normal CLI usage does not depend on it.
- `tests/test_benchmark.py`:
  deferred until the TS session/runtime layer is stable enough for meaningful benchmark measurements.
